...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    # Idempotent: if `auto_map` is already correct, no backup, no rewrite, no noise.
    # This was previously creating a fresh `config.json.back.<timestamp>` on every
    # eval invocation, polluting the checkpoint dir.
    if config.get("auto_map") == desired_auto_map:
        return

    # Config drifted (or was never patched) — back up once before overwriting.
...
def detect_memory_config(checkpoint_dir: str) -> Dict[str, Any]:
    """
    Inspect a checkpoint directory and infer mu-VLA memory hyperparameters.

    Resolution order:
        1. If `memory_meta.json` exists alongside the memory checkpoint, use it.
        2. Else if a `memory_module--*_checkpoint.pt` exists, peek at its
           state_dict to infer `num_mem_tokens` and `hidden_dim`. The cross-step
           update rule is inferred from the run name (heuristic: 'ema' substring
           → ema; otherwise tbptt). `ema_alpha` defaults to 0.1 if not specified.
        3. Else memory is absent → `{"use_memory": False}`.

    Returns a dict that overlays cleanly on `GenerateConfig`:
        {
            "use_memory": bool,
            "num_mem_tokens": int,         # only if use_memory=True
            "memory_update": "tbptt"|"ema",
            "ema_alpha": float,
            "attention_mask_mode": "custom"|"full",
        }
    """
    if not checkpoint_dir or not os.path.isdir(checkpoint_dir):
        return {"use_memory": False}

    # Look for memory checkpoint file.
    memory_files = [
        f for f in os.listdir(checkpoint_dir)
        if "memory_module" in f and "checkpoint" in f
    ]
    if not memory_files:
        return {"use_memory": False}

    # 1. memory_meta.json takes priority
    meta_path = os.path.join(checkpoint_dir, "memory_meta.json")
    if os.path.isfile(meta_path):
        try:
            with open(meta_path, "r") as f:
                meta = json.load(f)
            cfg_out = {"use_memory": True}
            if "num_mem_tokens" in meta:
                cfg_out["num_mem_tokens"] = int(meta["num_mem_tokens"])
            if "memory_update" in meta:
                cfg_out["memory_update"] = str(meta["memory_update"])
            if "ema_alpha" in meta:
                cfg_out["ema_alpha"] = float(meta["ema_alpha"])
            if "attention_mask_mode" in meta:
                cfg_out["attention_mask_mode"] = str(meta["attention_mask_mode"])
            cfg_out.setdefault("memory_update", "tbptt")
            cfg_out.setdefault("ema_alpha", 0.1)
            # Old checkpoints (no field) trained with custom mask — preserve behavior.
            cfg_out.setdefault("attention_mask_mode", "custom")
            return cfg_out
        except (OSError, ValueError, json.JSONDecodeError):
            pass  # fall through to inspection-based detection

    # 2. Inspect state_dict to derive num_mem_tokens
    memory_path = os.path.join(checkpoint_dir, memory_files[0])
    try:
        state_dict = torch.load(memory_path, weights_only=True, map_location="cpu")
        # Strip DDP `module.` prefix if present
        sd = {k.removeprefix("module."): v for k, v in state_dict.items()}
        if "initial_memory" not in sd:
            return {"use_memory": False}
        num_mem_tokens = int(sd["initial_memory"].shape[0])
    except Exception:
        return {"use_memory": False}

    # 3. Heuristic for memory_update based on run name
    name = os.path.basename(checkpoint_dir.rstrip("/")).lower()
    memory_update = "ema" if "ema" in name else "tbptt"
...
def get_memory_module(cfg: Any, llm_dim: int):
    """
    Get mu-VLA MemoryModule loaded from checkpoint.

    Args:
        cfg: Configuration object with use_memory, num_mem_tokens, pretrained_checkpoint
        llm_dim: Hidden dimension of the language model

    Returns:
        MemoryModule or None if use_memory is False
    """
    ...
    num_mem_tokens = getattr(cfg, "num_mem_tokens", 4)
    memory_module = MemoryModule(num_mem_tokens=num_mem_tokens, hidden_dim=llm_dim)
    memory_module = memory_module.to(torch.bfloat16).to(DEVICE)
    memory_module.eval()

    # Load checkpoint
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "memory_module")
    state_dict = load_component_state_dict(checkpoint_path)
...
def resize_image_for_policy(img: np.ndarray, resize_size: Union[int, Tuple[int, int]]) -> np.ndarray:
    ...
    Uses the same Lanczos3 resize that the training pipeline applies (via
    `dlimp.transforms.resize_image`). The legacy version of this function
    additionally encoded/decoded the image as JPEG before resizing — the
    intent was to mimic OpenX RLDS pipelines whose data is JPEG-stored.
    Both MIKASA and LIBERO RLDS datasets store images PNG-encoded
    (lossless), so the JPEG roundtrip introduced compression artifacts the
    model never saw at training, hurting eval (especially memory tasks
    where colors must be encoded into memory state at the cue step).

    Args:
        img: Numpy array containing the image (uint8, RGB).
    ...
    # Match training-time resize: dlimp.transforms.resize_image applies
    # tf.image.resize(method="lanczos3", antialias=True) directly on uint8,
    # then casts back to uint8. No JPEG roundtrip.
...
def get_vla_action(
    ...
    Args:
        ...
        use_memory_mask: Whether to use custom mu-VLA attention mask

    Returns:
        Tuple of (List[np.ndarray], new_memory_state) if memory is used,
    ...
    with torch.inference_mode():
        ...
        if action_head is None:
            ...
            if use_memory_mask and memory_state is not None:
                action, _, new_memory_state = result
            else:
                action, _ = result
                new_memory_state = None
    ...
    if use_memory_mask and memory_state is not None:
        return actions, new_memory_state
    return actions
