"""
visualize_mikasa_dataloader_3images.py

Same as visualize_mikasa_dataloader.py, but shows 3 images per timestep:
  1. Camera image (current step)
  2. Wrist image (current step)
  3. Initial image (first step of episode, constant throughout)

Applies the same ablation patch as finetune_mikasa_initial_obs.py.

Rows = batch positions (streams), columns = timesteps.
Each timestep cell shows 3 images side-by-side.

Usage:
    python utils/visualize_mikasa_dataloader_3images.py [options]

Examples:
    # Default: 4 streams, 50 steps, all 3 envs
    python utils/visualize_mikasa_dataloader_3images.py --n_steps 50

    # One env, custom output
    python utils/visualize_mikasa_dataloader_3images.py \\
        --n_steps 50 --batch_size 2 \\
        --env_names "remember_color_5_vla_v0" \\
        --output out/vis_3images.png
"""
...
from prismatic.vla.datasets.mikasa_episodic_dataset import (
    ...
    _chunk_actions_numpy,
    _load_single_episode_from_tf,
    _normalize_bounds_q99,
)
from prismatic.vla.constants import NUM_ACTIONS_CHUNK

import prismatic.vla.datasets.mikasa_episodic_dataset as _mod


# ── Apply ablation patches ────────────────────────────────────────────────────

_orig_make_stream = _mod.MIKASARoboVLAEpisodicDataset._make_stream


def _make_stream_with_initial(self, rng):
    """Patch _make_stream to yield initial_image from first frame (streaming version)."""
    env_iters = {
    ...
    while True:
        ...
        tf_episode = next(env_iters[env_name])
...
_mod.MIKASARoboVLAEpisodicDataset._make_stream = _make_stream_with_initial
...
class _IdentityTransform:
    """
    Passes raw step dict through unchanged — no tokenization.
    The dataset's __init__ and _make_stream are identical to training.
    """
...
# ── Visual constants ──────────────────────────────────────────────────────────
THUMB_W, THUMB_H = 64, 64  # smaller per-image to fit 3 side-by-side
IMAGES_PER_STEP = 3
CELL_W = THUMB_W * IMAGES_PER_STEP + 6  # 3 thumbs + 2px padding between + borders
...
COLOR_BOTH = (220, 160, 40)  # yellow — single-step episode
...
def _render_cell(step: dict) -> Image.Image:
    """Render 3 images (camera, wrist, initial) side-by-side in a cell."""
    ...
    # Render 3 images side-by-side
    images = [
        ("camera", step["image"]),
        ("wrist", step["wrist_image"]),
        ("init", step.get("initial_image", step["image"])),  # fallback to camera if no initial
    ]

    x_offset = 2
    for label_short, img_arr in images:
        img = Image.fromarray(img_arr).resize((THUMB_W, THUMB_H), Image.BILINEAR)

        # Draw border
        border = Image.new("RGB", (THUMB_W + 4, THUMB_H + 4), border_color)
        border.paste(img, (2, 2))
        cell.paste(border, (x_offset, 0))
        x_offset += THUMB_W + 4
...
def build_visualization(
    ...
    print("Creating MIKASARoboVLAEpisodicDataset with initial_image ablation...")
    ...
    # Collect n_steps per stream
    master_rng = np.random.default_rng(seed)

    def _with_step_counter(stream):
        """Injects per-episode step counter `t`."""
        t = 0
        for step in stream:
            if step["is_first"]:
                t = 0
    ...
    streams = [
        _with_step_counter(
            dataset._make_stream(
                np.random.default_rng(master_rng.integers(2**63))
            )
        )
    ...
    grid_h = COL_HEADER_H + batch_size * (CELL_H + PAD) + PAD + 16
    ...
    # Add note about 3 images
    note_y = grid_h - 28
    draw.text(
        (4, note_y),
        "Each cell shows: [camera | wrist | initial (t=0)]",
        fill=(150, 150, 150),
        font=font_sm,
    )
...
def main():
    ...
    parser.add_argument(
        "--env_names",
        default="shell_game_push_vla_v0,intercept_medium_vla_v0,remember_color_5_vla_v0,take_it_back_vla_v0,remember_shape_and_color_3x3_vla_v0",
    )
    parser.add_argument("--n_steps", type=int, default=50)
    parser.add_argument("--batch_size", type=int, default=4)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument(
        "--output", default="out/mikasa_dataloader_vis_3images.png"
    )
